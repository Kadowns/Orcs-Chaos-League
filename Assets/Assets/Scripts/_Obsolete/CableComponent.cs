...
public class CableComponent : MonoBehaviour
{
	...
	[SerializeField] private float _cableStrength = 200f;
	...
	void InitCableParticles()
	{
		...
		for (int pointIdx = 0; pointIdx <= segments; pointIdx++) {
			// Initial position
			Vector3 initialPosition = startPoint.position + (cableDirection * (initialSegmentLength * pointIdx));
	...
	void Update()
	{
		...
		float dist = (points[0].Position - points[segments-1].Position).sqrMagnitude;

		if (dist > cableLength * cableLength) {
						
			if (startPoint != null && endPoint != null) {
				Vector3 ropeTension = Vector3.zero;
				ropeTension = (startPoint.position - endPoint.position);

				endPoint.GetComponent<Rigidbody>().AddForce(ropeTension * _cableStrength * Time.deltaTime, ForceMode.Force);
			}  		
		}
	...
	{
        Vector3 gravityDisplacement = Vector3.zero;
